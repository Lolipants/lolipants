...
/** Minimal D1 mock for tailor assignment / pricing unit tests. */
export function createTailorMockDb(db: TailorMockTables): D1Database {
  ensureTailorTables(db);
  const prepare = (sql: string) => {
    ...
    const stmt = {
      ...
      async all<T>() {
        const rows = tailorMockSelect(db, sql, binds);
        return { results: (rows ?? []) as T[] };
      },
      async first<T>() {
        const row = tailorMockFirst(db, sql, binds);
